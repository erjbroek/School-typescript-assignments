...
export default class StartingScene extends Scene {
  ...
  public update(elapsed: number): Scene {
    if (this.readyGame === true) {
      return new Launch(this.maxX, this.maxY);
    } return null;
  }
  ...
  public render(canvas: HTMLCanvasElement): void {
    ...
    CanvasUtil.drawImage(canvas, this.title, canvas.width / 2, canvas.height / 23, this.title.width / 1.3, this.title.height / 1.3, 0);
